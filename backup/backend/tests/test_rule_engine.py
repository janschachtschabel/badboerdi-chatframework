...
class TestLiveExtraction:
    """Welle E (2026-05-17): ``extract_live`` ist ein No-op.  Alle
    Routing-Rules wurden global deaktiviert — sie laufen weiterhin als
    Shadow-Telemetrie, aber kein Effekt erreicht die Live-Pipeline.
    Daher liefert ``extract_live`` IMMER eine leere LiveDecision."""

    def test_live_extraction_is_global_noop(self):
        eng = RuleEngine([
        ...
        # Beide feuern weiterhin im Shadow-Record
        assert decision.intent_override == "SHADOW"
        assert decision.enforced_pattern_id == "PAT-LIVE"

        # Aber ``extract_live`` ist Welle-E-No-op → leere LiveDecision
        live = eng.extract_live(decision)
        assert live.is_noop()
        assert live.intent_override is None
        assert live.enforced_pattern_id is None
        assert live.fired_rules == []

    def test_extract_live_returns_noop_even_for_live_rules(self):
        """Selbst wenn ALLE Rules live=True sind, kommt nichts durch."""
        eng = RuleEngine([
