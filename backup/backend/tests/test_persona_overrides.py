...
def _apply_overrides(raw: dict, message: str) -> dict:
    ...
    PERSONA_SELF_ID_PATTERNS = [
        ...
        # Welle E (2026-05): P-VER deckt jetzt auch Politik ab (P-W-POL gemerged).
        ("P-VER", r"\b(ich\s+bin\s+(verwaltungs|beh[öo]rden|politiker(in)?|"
                  r"abgeordnete[r]?|multiplikator(in)?)|"
                  r"als\s+(verwaltungs|politiker(in)?)|"
                  ...
                  r"amtliche\s+daten|kennzahlen\s+f[üu]r\s+das\s+ministerium|"
...
SELF_ID_CASES = [
    ...
    ("Ich bin Politiker und suche Positionspapiere", "P-VER"),
    ("Für unsere Partei brauche ich aktuelle Bildungsdaten", "P-VER"),
