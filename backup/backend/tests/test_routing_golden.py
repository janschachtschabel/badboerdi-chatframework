"""Welle E (2026-05-17): Golden-Tests übersprungen.

Die Routing-Rules sind global auf no-op gesetzt (``extract_live`` gibt
immer eine leere LiveDecision zurück).  Die Golden-Tests, die
``decision.enforced_pattern_id`` aus einer Rule-Aktivierung erwarten,
können daher nicht mehr passen — die Live-Pipeline ignoriert sie.

Datei bleibt als Anker für zukünftiges gezieltes Re-Aktivieren einzelner
Rules.  Wenn das so weit ist, einzelne Tests entskippen.
"""
...
pytestmark = pytest.mark.skip(
    reason="Welle E: Alle Routing-Rules sind global deaktiviert "
           "(extract_live no-op). Golden-Tests testen abgeschaltete "
           "Funktionalität.",
)
