...
class RuleEngine:
    ...
    *first* (highest priority) one wins. List/dict effects merge.
    ...
    def extract_live(self, decision: RuleDecision) -> LiveDecision:
        ...
        Welle E (2026-05): GLOBALER KILL-SWITCH aktiv. Die Funktion gibt
        IMMER eine leere LiveDecision zurück, egal welche Rules gefeuert
        haben.  Die ``fired_rules`` der gesamten Decision bleiben für
        Telemetrie/Shadow-Logs erhalten, aber **kein** Rule-Effect erreicht
        die Live-Pipeline.

        Begründung: Wir testen das System im LLM-Hint-only-Modus.  Routing-
        Rules könnten den LLM-Hint überschreiben — das wollen wir gerade
        nicht.  Rules selbst bleiben als Daten in der YAML für späteres
        gezieltes Aktivieren erhalten.
        """
        return LiveDecision()
        return live
