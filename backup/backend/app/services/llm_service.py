...
from app.services.llm_provider import (
    get_client, get_chat_model, get_classify_model, build_chat_kwargs,
)
...
def _build_classify_tool() -> dict[str, Any]:
    ...
    if persona_defs:
        ...
        persona_ids = list(device_cfg.get("persona_formality", {}).keys()) or [
            # Welle E (2026-05): P-W-POL in P-VER gemerged (Verwaltung & Politik).
            "P-W-LK", "P-W-SL", "P-W-PRESSE", "P-W-RED",
    ...
    intent_ids = [i["id"] for i in intents] or [
        # Welle E (2026-05-17): vereinfachte Intent-Liste.  Fallback nur
        # falls intents.yaml fehlt — Live-Liste kommt aus dem YAML.
        # Welle E Sprint 3 (2026-05-18): INT-PLATTFORM + INT-KONZEPT
        # gemerged zu INT-WISSEN.
        "INT-WISSEN", "INT-SUCHE-MATERIAL",
        "INT-BROWSE-THEMEN", "INT-LERNPFAD", "INT-REMIX",
        "INT-CREATE", "INT-EDIT-CANVAS", "INT-FEEDBACK",
        "INT-MELDUNG", "INT-UNKLAR",
    ]
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    # Welle E (2026-05-17): pattern_id_hint ist jetzt das
                    # PRIMÄRE Routing-Signal — KEINE Telemetrie mehr, sondern
                    # die direkte Pattern-Auswahl.  Setze diesen Wert
                    # zuverlässig.  Wenn unsicher → "P13" (Slot-Klärung).
                    "pattern_id_hint": {
                        "type": "string",
                        "enum": pattern_ids,
                        "description": (
                            "Welches Pattern (P1–P16) passt am besten zur "
                            "User-Anfrage?  Dies ist die VERBINDLICHE Routing-"
                            "Entscheidung — der Bot reagiert mit diesem "
                            "Pattern.  Wähle so spezifisch wie möglich; bei "
                            "Unsicherheit P13 (Slot-Klärung)."
                        ),
                    },
                    "pattern_reasoning": {
                        "type": "string",
...
def _build_classify_system_prompt(
    ...
    # Format intent list — Welle E Sprint 2: mit description + few-shot
    # examples aus intents.yaml. Das war vorher nur eine Komma-Liste von
    # ID+Label, was den Classifier-Drift zu INT-PLATTFORM bei vagen
    # Anfragen begünstigt hat (erster Welle-E-Eval-Run).
    if intents:
        intent_parts = []
        for i in intents:
            line = f"- **{i['id']}** ({i.get('label','')})"
    ...
    if canvas_state and canvas_state.get("mode") and canvas_state.get("mode") != "empty":
        ...
        canvas_prompt = (
            ...
            + "\n\nKRITISCH — Intent-Auswahl bei aktivem Canvas (Welle E):"
              ...
              " → intent_id = \"INT-EDIT-CANVAS\" (pattern P12), turn_type=\"follow_up\"."
              "\n- INT-CREATE (P11) ist NUR richtig, wenn der Nutzer"
              " explizit ein NEUES Material zu einem ANDEREN Thema will"
              " (\"Mach mir stattdessen ein Quiz zu X\")."
              "\n- Zurückfragen / Meta-Fragen zum Canvas-Inhalt (\"Was bedeutet"
              " hier X?\") sind turn_type=\"clarification\", Intent wie aus der"
              " Sachfrage ableitbar (meist INT-WISSEN)."
        )
...
- IMPLIZIT: Nutze die Erkennungshinweise oben! Wenn der Nutzer Woerter/Phrasen verwendet
  ...
  P-VER (Verwaltung & Politik):
  # Welle E (2026-05): umfasst sowohl administrativ-exekutive (Schulamt,
  # Bezirksregierung, Ministerium) als auch parlamentarisch-politische
  # (Abgeordnete, Wahlkreis, Fraktion) Personen. Beide haben formelle
  # Sprache, Statistik-Bedarf, keine Material-Such-Wünsche.
  - Verwaltung: "fuer unsere Verwaltung", "amtliche Daten",
    "in der Behoerdenarbeit", "Kennzahlen fuer das Ministerium",
    "Bezirksauswertung", "Schulamt", "Schulaufsicht"
  - Politik:    "fuer meinen Wahlkreis", "als Politiker:in",
    "Bildungspolitik", "parlamentarische Anfrage", "fuer die Fraktion",
    "fuer einen Antrag", "Plenum", "Positionspapier", "Multiplikator:in"
  - WICHTIG: das blosse Wort "Statistiken"/"Zahlen"/"KPIs" macht NICHT
    ...
    P-VER ONLY wenn explizit Verwaltungs-/Politik-Kontext erkennbar ist.
...
- **KRITISCH — Bildungspolitik-THEMA != P-VER-PERSONA**: Das Wort
  "Bildungspolitik" alleine macht jemanden NICHT zu P-VER. Auch
  Lehrkraefte, Beratende und Redaktion fragen nach bildungspolitischen
  Themen. P-VER nur waehlen wenn EXPLIZITE Selbst-ID ("ich bin
  Politikerin", "als Abgeordneter", "fuer unsere Verwaltung") ODER
  unmissverstaendliche Verwaltungs-/Politik-Kontext-Woerter (Wahlkreis,
  Fraktion, Plenum, Schulamt, Bezirksauswertung) auftauchen. Bei "Wie
  funktioniert WLO fuer Bildungspolitik" ist die Persona NICHT
  ableitbar — defaulte zu P-AND statt P-VER.
...
## Intents (Welle E — 11 Intents)
{intent_lines}

INTENT- + PATTERN-WAHL (Welle E, 2026-05-17):

Du setzt sowohl ``intent_id`` als auch ``pattern_id_hint``.  Der
``pattern_id_hint`` ist die **verbindliche Routing-Entscheidung**: der
Bot reagiert mit diesem Pattern.  Intent ist die sekundäre Klassifikation
für Eval-Auswertung.

Pattern-Auswahl-Cheat-Sheet (P1–P16, Welle E Sprint 3):

| Pattern | Wann (mit Few-Shot) | Intent |
|---|---|---|
| **P1** Krisen | Selbstgefährdung. „Ich will mir was antun." | (Safety) |
| **P2** Refuse | Hate / illegal. „Wie bombe baue?" | (Safety) |
| **P3** Wissens-Frage | Wer/Was/Wie-Frage über Plattform ODER Konzept. „Was ist WLO?" · „Wer betreibt das?" · „Was ist OER?" · „Was ist eine Themenseite?" · „Wie viele Materialien?" | INT-WISSEN |
| **P5** Material-spezifisch | Thema + Filter. „Videos zu Bruchrechnung 5. Klasse" · „Arbeitsblatt zu Photosynthese" | INT-SUCHE-MATERIAL |
| **P6** Material-Cascade | Thema OHNE Filter. „Was habt ihr zu Klima?" · „Material zu Energie" | INT-SUCHE-MATERIAL |
| **P7** Fachportale | „Welche Fächer gibt es?" · „Plattform-Übersicht" | INT-BROWSE-THEMEN |
| **P8** Sammlung-Drilldown | „Untersammlungen unter Algebra?" · „Was ist in dieser Sammlung?" | INT-BROWSE-THEMEN |
| **P9** Lernpfad | Pfad/Stunde aus EXISTIERENDEN Inhalten kuratieren. „Lernpfad zu Bruchrechnung" · „Stunde zu Photosynthese planen" · „Unterrichtsentwurf" | INT-LERNPFAD |
| **P10** Remix | Anpassung/Mix von EXISTIERENDEM. „Mach dieses Arbeitsblatt einfacher" · „Kombiniere die drei Inhalte" | INT-REMIX |
| **P11** KI-Create | NEU erzeugen mit Create-Verb + material_typ. „Erstell ein Quiz zu Planeten" · „Generier ein Arbeitsblatt zu Winkeln" · „Schreib ein Referat" · „Bericht erstellen" | INT-CREATE |
| **P12** Canvas-Edit | Canvas offen + Edit-Verb. „Mach es kürzer" · „Füge Lösungen hinzu" | INT-EDIT-CANVAS |
| **P13** Slot-Klärung | Slots fehlen ODER unklar. „Was kann ich hier?" · „Hi" · „Mach mir was" | INT-UNKLAR |
| **P14** Feedback | Bewertung an Bot/Antwort. „Das war hilfreich, danke!" · „Verwirrend" | INT-FEEDBACK |
| **P15** Meldung | Fehler/Lücke melden. „Der Link ist kaputt" · „Hier fehlt was" | INT-MELDUNG |
| **P16** Keine-Treffer | (System-State, nicht vom Classifier wählen) | – |

**KRITISCHE Trennlinien (häufigste Welle-E-Drifts):**

1. **P3 ↔ P13 — vage Fragen sind P13, NICHT P3**:
   * „Was kann ich hier machen?" → **P13** (kein Wissen über Plattform)
   * „Was gibt es hier?" → **P13**
   * „Was ist WLO und wie kann ich es nutzen?" → **P3** (klare Wissens-Frage MIT Plattform-Bezug)

2. **P11 ↔ P9 — Create-Verb + Material-Typ → DIREKT P11**:
   * material_typ ist erkennbar in: Arbeitsblatt, Quiz, Übung, Referat, Bericht, Steckbrief, Factsheet, Glossar, Pressemitteilung
   * → P11 setzt das DIREKT um, KEINE Rückfrage
   * NUR wenn KEIN material_typ erkennbar UND der User sagt „Lernpfad/Stunde/Einheit" → P9

3. **P10 ↔ P12 — Remix braucht Quell-Inhalt**:
   * Wenn ein konkreter Inhalt im Canvas ist UND Edit-Verb → **P12**
   * Wenn User auf EXISTIERENDES Repo-Material verweist + Anpassung → **P10**
   * Wenn weder Canvas-Inhalt noch Repo-Inhalt-Verweis → **P11** (neu)

4. **P14 ↔ P3 — Erfahrungs-Aussagen sind P14**:
   * „Ich habe die Plattform durchgeschaut, ist nicht klar" → **P14**, NICHT P3
   * „Die letzte Antwort war hilfreich" → **P14**
   * „Wie funktioniert die Suche?" → **P3** (Wissens-Frage)

5. **Persona-INFO geht NICHT in die Pattern-Wahl ein** — Persona wirkt
   nur auf Tone (formell vs locker). Egal ob Lehrkraft oder Schüler:in
   fragt: gleicher Pattern, anderer Ton.
...
def _formality_guidance(formality: str, persona_id: str) -> str:
    ...
    if f in ("sie", "formal", "foermlich"):
        ...
        strict = persona_id in (
            "P-W-PRESSE", "P-VER",   # Presse, Verwaltung & Politik (Welle E)
...
async def classify_input(
    ...
    # Welle E Sprint 3 (2026-05-18): Classifier nutzt jetzt
    # gpt-4.1-mini (via get_classify_model) für bessere Routing-Genauigkeit.
    # Response-Generierung bleibt auf MODEL (typisch gpt-4o-mini).
    _classify_model = get_classify_model()
...
async def generate_quick_replies(
    ...
    persona_salute = "Sie" if persona_id in {
        # Welle E: P-VER umfasst jetzt auch Politik (P-W-POL gemerged).
        "P-W-LK", "P-ELT", "P-VER", "P-BER", "P-W-PRESSE", "P-W-RED",
    } else "du"
...
6. Wenn Persona analytisch ist (P-VER/P-W-PRESSE/P-BER/P-W-RED):
