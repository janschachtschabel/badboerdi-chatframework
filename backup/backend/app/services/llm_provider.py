...
LLM_CLASSIFY_MODEL override classifier-only model (Welle E Sprint 3:
                   default ``gpt-4.1-mini`` für bessere Routing-Genauigkeit
                   bei der ``classify_input``-Phase; Chat-/Response-Phase
                   bleibt auf LLM_CHAT_MODEL).
...
def get_classify_model() -> str:
    """Welle E Sprint 3 (2026-05-18): separates Modell für den
    classify_input-Call.

    Default: ``gpt-4.1-mini`` (besseres Routing-Verhalten bei Pattern-/
    Intent-Wahl als gpt-4o-mini, ähnliche Kosten/Latenz).  Kann via
    ``LLM_CLASSIFY_MODEL`` überschrieben werden.  Fällt auf
    ``get_chat_model()`` zurück, wenn nicht-OpenAI-Provider aktiv ist
    (B-API kennt das Modell evtl. nicht).
    """
    override = (os.getenv("LLM_CLASSIFY_MODEL") or "").strip()
    if override:
        return override
    if get_provider() == "openai":
        return "gpt-4.1-mini"
    # Bei B-API-Providern: dasselbe Modell wie für Chat (sonst
    # Konfigurations-Drift). Wer das override will, setzt
    # LLM_CLASSIFY_MODEL explizit.
