...
async def _chat_impl(
    ...
    # Welle D Sprint 2: pattern_selection_mode aus environment durchreichen.
    # None = YAML-Default greift. Eval-Runs setzen das explizit auf
    # "llm_hint_authoritative" für A/B-Vergleich.
    _selection_mode = getattr(req.environment, "pattern_selection_mode", None)
    ...
    tracer.end({
        ...
        "persona_loosening": pattern_output.get("persona_loosening"),
    })
    ...
    debug = DebugInfo(
        ...
        phase3_modulations={
            ...
            "tie_breaker": pattern_output.get("tie_breaker"),
            # Persona-Loosening (Welle D Shadow-A/B): Counterfactual-Score
            # ohne persona-typische Trait-Signals. None = nicht ausgelöst.
            # Bei mode=shadow loggen wir would_change_winner; bei mode=live
            # wird der Counterfactual-Winner ggf. zum echten Winner.
            "persona_loosening": pattern_output.get("persona_loosening"),
            # LLM-Hint Authoritative (Welle D Sprint 2): Wenn der Pattern-
            # Selection-Mode auf "llm_hint_authoritative" stand, wird hier
            # geloggt welche Engine-Gates das gewählte Pattern verletzt
            # hätte. Erlaubt A/B-Vergleich gegen klassische Engine.
            "llm_hint_authoritative": pattern_output.get("llm_hint_authoritative"),
