...
class StartRequest(BaseModel):
    ...
    turns_per_conv: int = Field(3, ge=1, le=10)
    config_slug: str = ""
    # Welle D Shadow-A/B addons.  Beide default False, damit Bestands-Runs
    # genau dasselbe Verhalten wie vor der Änderung haben.
    include_cross_persona: bool = Field(
        False,
        description="Zusätzliche kuratierte atypische Persona×Intent-Combos "
                    "aus 01-base/cross-persona-scenarios.yaml mitlaufen lassen.",
    )
    enable_pattern_rejudge: bool = Field(
        False,
        description="Bei engine_pattern != llm_hint einen zweiten Judge-Call "
                    "(Forced-Choice) ausführen, um Engine vs LLM-Hint zu vergleichen. "
                    "Verdoppelt Judge-Tokens nur für Disagreement-Turns.",
    )
    # Welle D Sprint 2: A/B-Modus für Pattern-Auswahl.
    pattern_selection_mode: str = Field(
        "engine",
        pattern="^(engine|engine_with_hint_tiebreaker|llm_hint_authoritative)$",
        description=(
            "Pattern-Auswahl-Strategie für diesen Run. "
            "'engine' = klassisches 3-Phasen-Routing (Default). "
            "'llm_hint_authoritative' = LLM-Hint überschreibt Engine vollständig "
            "(Diagnostik-Modus, nur für Eval — Gates werden zu Telemetrie)."
        ),
    )
...
async def start_run(req: StartRequest) -> dict[str, Any]:
    ...
    # Welle D Sprint 2: "engine" ist der Default — wir reichen es trotzdem
    # explizit als pattern_selection_mode durch, damit der Mode in der
    # Run-Summary für Vergleichs-Auswertungen lesbar ist.
    _sel_mode = req.pattern_selection_mode if req.pattern_selection_mode != "engine" else None
    asyncio.create_task(execute_run(
        ...
        include_cross_persona=req.include_cross_persona,
        enable_pattern_rejudge=req.enable_pattern_rejudge,
        pattern_selection_mode=_sel_mode,
    ))
    return {
        "run_id": run_id,
        "status": "running",
