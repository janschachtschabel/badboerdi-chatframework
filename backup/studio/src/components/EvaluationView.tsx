...
interface ClassificationMetrics {
  ...
  // Welle D Shadow-A/B — Tie-Breaker (live) und Persona-Loosening (counterfactual).
  tie_breaker?: {
    evaluated_turns?: number;
    would_apply_count?: number;
    would_apply_rate?: number;
    would_overrides?: Record<string, number>;   // shadow "from->to"
    applied_count?: number;
    applied_rate?: number;
    overrides?: Record<string, number>;          // live "from->to"
  };
  persona_loosening?: {
    evaluated_turns?: number;
    would_change_count?: number;
    would_change_rate?: number;
    would_changes?: Record<string, number>;      // shadow "from->to"
    applied_count?: number;
    applied_rate?: number;
  };
  // Welle D addon — Cross-Persona-Scenarios und Strategy-Showdown.
  cross_persona?: {
    judged_turns?: number;
    avg_score?: number;
    engine_pattern_judge_ok_count?: number;
    engine_pattern_judge_ok_rate?: number;
    persona_correct_rate?: number;
    intent_correct_rate?: number;
  ...
  strategy_showdown?: {
    evaluated_turns?: number;
    engine_wins?: number;
    llm_hint_wins?: number;
    ties?: number;
    engine_win_rate?: number;
    llm_hint_win_rate?: number;
    engine_avg_fit?: number;
    llm_hint_avg_fit?: number;
    engine_wins_by_pair?: Record<string, number>;
    llm_hint_wins_by_pair?: Record<string, number>;
  };
  llm_hint_authoritative?: {
    evaluated_turns?: number;
    gate_violation_count?: number;
    gate_violation_rate?: number;
...
function NewRunModal({ onClose, onStarted }: { onClose: () => void; onStarted: () => void }) {
  ...
  const [error, setError] = useState<string | null>(null);
  // Welle E (2026-05-17): Pattern-Auswahl läuft immer per LLM-Hint
  // (kein A/B-Toggle mehr). Diese zwei Toggles bleiben sinnvoll, weil
  // sie zusätzliche Test-Tiefe geben.
  const [includeCrossPersona, setIncludeCrossPersona] = useState(false);
  const [enablePatternRejudge, setEnablePatternRejudge] = useState(false);
    ...
    try {
      const r = await fetch('/api/eval/runs', {
        ...
        body: JSON.stringify({
          ...
          scenarios_per_combo: scenariosPerCombo,
          turns_per_conv: turnsPerConv,
          include_cross_persona: includeCrossPersona,
          enable_pattern_rejudge: enablePatternRejudge,
        }),
      });
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ background: '#fff', borderRadius: 8, maxWidth: 720, width: '100%', maxHeight: '90vh', overflowY: 'auto', padding: 24 }}>
        ...
        {/* Welle E (2026-05): Test-Add-ons.  Engine-vs-LLM-A/B ist obsolet,
            Pattern-Wahl ist immer LLM-Hint-gesteuert. */}
        <div style={{
          ...
          <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, cursor: 'pointer', marginBottom: 6 }}>
            <input
              type="checkbox"
              checked={includeCrossPersona}
              onChange={e => setIncludeCrossPersona(e.target.checked)}
              disabled={mode === 'conversations'}
              style={{ marginTop: 2 }}
            />
            <div style={{ fontSize: 12 }}>
              <strong>Cross-Persona-Scenarios mitlaufen lassen</strong>
              <div style={{ color: '#6B7280' }}>
                Zusätzlich zur Persona×Intent-Matrix die kuratierten atypischen
                Combos aus <code>01-base/cross-persona-scenarios.yaml</code> testen
                (z.B. Schüler:in × Unterrichtsplanung). Eigener Score-Block im Ergebnis.
                {mode === 'conversations' && (
                  <span style={{ color: '#B91C1C', display: 'block', marginTop: 2 }}>
                    Nur in Modus „Szenarien" oder „Beides" verfügbar.
                  </span>
                )}
              </div>
            </div>
          ...
          <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, cursor: 'pointer' }}>
            <input
              type="checkbox"
              checked={enablePatternRejudge}
              onChange={e => setEnablePatternRejudge(e.target.checked)}
              style={{ marginTop: 2 }}
            />
            <div style={{ fontSize: 12 }}>
              <strong>LLM-Hint Re-Judge (Strategy-Showdown)</strong>
              <div style={{ color: '#6B7280' }}>
                Bei <code>engine_pattern ≠ llm_hint</code> einen zweiten Judge-Call
                ausführen (Forced-Choice). Misst direkt, ob Regelsystem oder
                LLM-Hint besser performt. Verdoppelt Judge-Tokens nur auf
                Disagreement-Turns (~20-30%).
              </div>
            </div>
          </label>
        </div>
...
function ClassificationMetricsView({ metrics }: { metrics?: ClassificationMetrics }) {
  ...
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {/* Welle D Shadow-A/B — Tie-Breaker + Persona-Loosening */}
      <ShadowImpactPanel
        tieBreaker={metrics.tie_breaker}
        personaLoosening={metrics.persona_loosening}
      />

      {/* Welle D addons — Cross-Persona Aggregat + Strategy-Showdown */}
      <CrossPersonaPanel data={metrics.cross_persona} />
      <StrategyShowdownPanel data={metrics.strategy_showdown} />
      <AuthoritativePanel data={metrics.llm_hint_authoritative} />
...
// ── Welle D Shadow-A/B: Tie-Breaker + Persona-Loosening ──────────
//
// Zeigt in einem Block, was die zwei Shadow-Rules über den Eval-Run hinweg
// gemessen haben.  Wenn beide nicht aktiv (enabled=false) oder noch keine
// Turns gelaufen sind, blendet sich der Block aus.
function ShadowImpactPanel({
  tieBreaker, personaLoosening,
}: {
  tieBreaker?: ClassificationMetrics['tie_breaker'];
  personaLoosening?: ClassificationMetrics['persona_loosening'];
}) {
  const tbActive = !!tieBreaker && (tieBreaker.evaluated_turns || 0) > 0;
  const plActive = !!personaLoosening && (personaLoosening.evaluated_turns || 0) > 0;
  if (!tbActive && !plActive) return null;

  const tbEval = tieBreaker?.evaluated_turns || 0;
  const tbWould = tieBreaker?.would_apply_count || 0;
  const tbWouldRate = tieBreaker?.would_apply_rate || 0;
  const tbApplied = tieBreaker?.applied_count || 0;
  const tbAppliedRate = tieBreaker?.applied_rate || 0;
  const tbShadowEdges = Object.entries(tieBreaker?.would_overrides || {})
    .sort(([, a], [, b]) => b - a);
  const tbLiveEdges = Object.entries(tieBreaker?.overrides || {})
    .sort(([, a], [, b]) => b - a);

  const plEval = personaLoosening?.evaluated_turns || 0;
  const plWould = personaLoosening?.would_change_count || 0;
  const plWouldRate = personaLoosening?.would_change_rate || 0;
  const plApplied = personaLoosening?.applied_count || 0;
  const plShadowEdges = Object.entries(personaLoosening?.would_changes || {})
    .sort(([, a], [, b]) => b - a);
  ...
  const labelStyle: React.CSSProperties = {
    padding: '4px 8px', color: '#374151', fontSize: 12,
  };

  return (
    <div style={{
      marginBottom: 16, padding: 12,
      background: '#FAF5FF',  // dezenter Lila-Hintergrund, hebt Shadow-Block ab
      border: '1px solid #E9D5FF', borderRadius: 6,
    }}>
      <h4 style={{ marginTop: 0, marginBottom: 4 }}>
        Shadow-Rules (Welle D A/B)
      </h4>
      <div style={{ color: '#6B7280', fontSize: 11, marginBottom: 10 }}>
        Tie-Breaker und Persona-Lockerung. Im Shadow-Modus loggen sie nur,
        was passieren würde — der Engine-Winner bleibt unverändert, bis
        `live: true` in der jeweiligen YAML gesetzt wird.
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        {/* Tie-Breaker */}
        <div>
          <h5 style={{ marginTop: 0, marginBottom: 6, fontSize: 12, color: '#5B21B6' }}>
            Tie-Breaker (LLM-Hint Override)
          </h5>
          {tbActive ? (
            <table style={{ borderCollapse: 'collapse', width: '100%' }}>
              <tbody>
                <tr style={{ background: '#fff' }}>
                  <td style={labelStyle}>Turns mit Tie-Race</td>
                  <td style={{ ...cellStyle, textAlign: 'right' }}>{tbEval}</td>
                </tr>
                <tr>
                  <td style={labelStyle}>Würde übersteuern</td>
                  <td style={{ ...cellStyle, textAlign: 'right' }}>
                    {tbWould} ({(tbWouldRate * 100).toFixed(0)}%)
                  </td>
                </tr>
                <tr style={{ background: '#fff' }}>
                  <td style={labelStyle}>Tatsächlich übersteuert (live)</td>
                  <td style={{ ...cellStyle, textAlign: 'right' }}>
                    {tbApplied} ({(tbAppliedRate * 100).toFixed(0)}%)
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div style={{ color: '#6B7280', fontSize: 11 }}>
              Keine Tie-Races im Run — keine Konflikte zwischen Engine-Winner
              und LLM-Hint.
            </div>
          )}

          {tbShadowEdges.length > 0 && (
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 4 }}>
                Shadow-Overrides (würde wechseln zu)
              </div>
              <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
                <tbody>
                  {tbShadowEdges.slice(0, 8).map(([edge, n]) => (
                    <tr key={edge}>
                      <td style={{ padding: '2px 6px', fontFamily: 'monospace' }}>{edge}</td>
                      <td style={{ padding: '2px 6px', textAlign: 'right', fontFamily: 'monospace' }}>{n}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {tbLiveEdges.length > 0 && (
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 4 }}>
                Live-Overrides (tatsächlich gewechselt)
              </div>
              <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
                <tbody>
                  {tbLiveEdges.slice(0, 8).map(([edge, n]) => (
                    <tr key={edge}>
                      <td style={{ padding: '2px 6px', fontFamily: 'monospace' }}>{edge}</td>
                      <td style={{ padding: '2px 6px', textAlign: 'right', fontFamily: 'monospace' }}>{n}</td>
                    </tr>
        ...
        <div>
          <h5 style={{ marginTop: 0, marginBottom: 6, fontSize: 12, color: '#5B21B6' }}>
            Persona-Lockerung (Score-Counterfactual)
          </h5>
          {plActive ? (
            <table style={{ borderCollapse: 'collapse', width: '100%' }}>
              <tbody>
                <tr style={{ background: '#fff' }}>
                  <td style={labelStyle}>Turns mit Persona-Traits</td>
                  <td style={{ ...cellStyle, textAlign: 'right' }}>{plEval}</td>
                </tr>
                <tr>
                  <td style={labelStyle}>Würde Winner ändern</td>
                  <td style={{ ...cellStyle, textAlign: 'right' }}>
                    {plWould} ({(plWouldRate * 100).toFixed(0)}%)
                  </td>
                </tr>
                <tr style={{ background: '#fff' }}>
                  <td style={labelStyle}>Tatsächlich geändert (live)</td>
                  <td style={{ ...cellStyle, textAlign: 'right' }}>{plApplied}</td>
                </tr>
              </tbody>
            </table>
          ) : (
            <div style={{ color: '#6B7280', fontSize: 11 }}>
              Keine persona-typischen Trait-Signals im Run.
            </div>
          )}

          {plShadowEdges.length > 0 && (
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 4 }}>
                Shadow-Wechsel (würde Pattern wechseln zu)
              </div>
              <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
                <tbody>
                  {plShadowEdges.slice(0, 8).map(([edge, n]) => (
                    <tr key={edge}>
                      <td style={{ padding: '2px 6px', fontFamily: 'monospace' }}>{edge}</td>
                      <td style={{ padding: '2px 6px', textAlign: 'right', fontFamily: 'monospace' }}>{n}</td>
                    </tr>
...
// ── Welle D addon: Cross-Persona-Aggregat ──────────────────────────
//
// Zeigt einen separaten Score-Block für die kuratierten atypischen
// Persona×Intent-Combos.  Blendet sich aus, wenn judged_turns=0 (Run hatte
// keine cross-persona-Scenarios).
function CrossPersonaPanel({
  data,
}: {
  data?: ClassificationMetrics['cross_persona'];
}) {
  const judged = data?.judged_turns || 0;
  if (judged === 0) return null;

  const avg = data?.avg_score || 0;
  const okRate = data?.engine_pattern_judge_ok_rate || 0;
  const personaRate = data?.persona_correct_rate || 0;
  const intentRate = data?.intent_correct_rate || 0;
  const usage = Object.entries(data?.pattern_usage || {})
    .sort(([, a], [, b]) => b - a)
    .slice(0, 8);

  const rateColor = (r: number) => r >= 0.7 ? '#10B981' : r >= 0.5 ? '#F59E0B' : '#EF4444';

  return (
    <div style={{
      marginBottom: 16, padding: 12,
      background: '#FFFBEB',  // sanftes Gelb — hebt Edge-Case-Block ab
      border: '1px solid #FDE68A', borderRadius: 6,
    }}>
      <h4 style={{ marginTop: 0, marginBottom: 4 }}>
        Cross-Persona-Aggregat (Welle D Edge-Cases)
      </h4>
      <div style={{ color: '#6B7280', fontSize: 11, marginBottom: 10 }}>
        Untypische Persona×Intent-Kombinationen (z.B. Schüler:in plant Unterricht,
        Eltern erstellen Material).  Separater Score, um zu messen, wie robust das
        System ist, wenn das Persona-Gate eigentlich blocken würde.
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 12 }}>
        <Stat label="Bewertete Turns" value={String(judged)} />
        <Stat label="Avg-Score" value={avg.toFixed(2)} color={rateColor(avg)} />
        <Stat label="Pattern-OK" value={`${(okRate * 100).toFixed(0)}%`} color={rateColor(okRate)} />
        <Stat label="Persona richtig" value={`${(personaRate * 100).toFixed(0)}%`} color={rateColor(personaRate)} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <Stat label="Intent richtig" value={`${(intentRate * 100).toFixed(0)}%`} color={rateColor(intentRate)} />
      </div>

      {usage.length > 0 && (
        <div style={{ marginTop: 12 }}>
          <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 4 }}>
            Top-Patterns in Cross-Persona-Turns
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {usage.map(([pid, n]) => (
              <span key={pid} style={{
                ...
                {pid} <span style={{ color: '#6B7280' }}>×{n}</span>
              </span>
            ))}
          </div>
        </div>
...
// ── Welle D addon: Strategy-Showdown (Engine vs LLM-Hint) ──────────
//
// Zeigt das Re-Judge-Ergebnis: bei wievielen Disagreement-Turns hat der
// Judge sich für Engine bzw LLM-Hint entschieden.  Blendet sich aus,
// wenn kein Re-Judge gelaufen ist.
function StrategyShowdownPanel({
  data,
}: {
  data?: ClassificationMetrics['strategy_showdown'];
}) {
  const evaluated = data?.evaluated_turns || 0;
  if (evaluated === 0) return null;

  const engineWins = data?.engine_wins || 0;
  const llmWins = data?.llm_hint_wins || 0;
  const ties = data?.ties || 0;
  const engineRate = data?.engine_win_rate || 0;
  const llmRate = data?.llm_hint_win_rate || 0;
  const engineFit = data?.engine_avg_fit || 0;
  const llmFit = data?.llm_hint_avg_fit || 0;
  const engineByPair = Object.entries(data?.engine_wins_by_pair || {})
    .sort(([, a], [, b]) => b - a).slice(0, 6);
  const llmByPair = Object.entries(data?.llm_hint_wins_by_pair || {})
    .sort(([, a], [, b]) => b - a).slice(0, 6);

  // Winner-Color: grün für die Strategie mit Mehrheit
  const engineColor = engineWins > llmWins ? '#10B981' : engineWins < llmWins ? '#9CA3AF' : '#6B7280';
  const llmColor = llmWins > engineWins ? '#10B981' : llmWins < engineWins ? '#9CA3AF' : '#6B7280';
  ...
  return (
    <div style={{
      ...
      <h4 style={{ marginTop: 0, marginBottom: 4 }}>
        Strategy-Showdown (Engine vs LLM-Hint)
      </h4>
      <div style={{ color: '#6B7280', fontSize: 11, marginBottom: 10 }}>
        Bei Pattern-Disagreement zwischen Regelsystem (Engine) und LLM-Hint
        entscheidet ein zweiter Judge-Call, welche Strategie objektiv besser
        gepasst hätte (Forced-Choice).  Antwort auf: „Was performt besser?"
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 12 }}>
        <Stat label="Disagreement-Turns" value={String(evaluated)} />
        <Stat
          label={`Engine gewinnt`}
          value={`${engineWins} (${(engineRate * 100).toFixed(0)}%)`}
          color={engineColor}
        />
        <Stat
          label={`LLM-Hint gewinnt`}
          value={`${llmWins} (${(llmRate * 100).toFixed(0)}%)`}
          color={llmColor}
        />
        <Stat label="Unentschieden" value={String(ties)} />
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
        <Stat label="Engine — Avg-Eignung" value={`${engineFit.toFixed(2)} / 2`} color={engineColor} />
        <Stat label="LLM-Hint — Avg-Eignung" value={`${llmFit.toFixed(2)} / 2`} color={llmColor} />
      </div>

      {(engineByPair.length > 0 || llmByPair.length > 0) && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {engineByPair.length > 0 && (
            <div>
              <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 4 }}>
                Engine-Wins by Pattern-Pair
              </div>
              <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
                <tbody>
                  {engineByPair.map(([pair, n]) => (
                    <tr key={pair}>
                      <td style={{ padding: '2px 6px', fontFamily: 'monospace' }}>{pair}</td>
                      <td style={{ padding: '2px 6px', textAlign: 'right', fontFamily: 'monospace' }}>{n}</td>
                    </tr>
                  ))}
                </tbody>
          ...
          {llmByPair.length > 0 && (
            <div>
              <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 4 }}>
                LLM-Hint-Wins by Pattern-Pair
              </div>
              <table style={{ borderCollapse: 'collapse', fontSize: 11 }}>
                <tbody>
                  {llmByPair.map(([pair, n]) => (
                    <tr key={pair}>
                      <td style={{ padding: '2px 6px', fontFamily: 'monospace' }}>{pair}</td>
                      <td style={{ padding: '2px 6px', textAlign: 'right', fontFamily: 'monospace' }}>{n}</td>
                    </tr>
...
// ── Welle D Sprint 2: LLM-Hint Authoritative ───────────────────────
//
// Zeigt Telemetrie für Runs im A/B-Modus "llm_hint_authoritative".  Wenn
// keine Turns im Authoritative-Modus liefen (Engine-Run), blendet sich
// das Panel aus.  Hauptmetrik: wie oft hätte das LLM-gewählte Pattern
// Engine-Gates verletzt (Persona/State/Intent/Precondition).
function AuthoritativePanel({
  data,
}: {
  data?: ClassificationMetrics['llm_hint_authoritative'];
}) {
  const turns = data?.evaluated_turns || 0;
  if (turns === 0) return null;

  const violations = data?.gate_violation_count || 0;
  const violationRate = data?.gate_violation_rate || 0;
  const kinds = Object.entries(data?.violation_kinds || {})
    .sort(([, a], [, b]) => b - a);
  const usage = Object.entries(data?.pattern_usage || {})
    .sort(([, a], [, b]) => b - a).slice(0, 10);

  const violationColor = violationRate >= 0.4 ? '#EF4444'
    : violationRate >= 0.2 ? '#F59E0B' : '#10B981';

  return (
    <div style={{
      marginBottom: 16, padding: 12,
      background: '#FEF3C7',  // Diagnostik-Warn-Gelb
      border: '1px solid #FCD34D', borderRadius: 6,
    }}>
      <h4 style={{ marginTop: 0, marginBottom: 4 }}>
        LLM-Hint Authoritative (Diagnostik-Modus)
      </h4>
      <div style={{ color: '#92400E', fontSize: 11, marginBottom: 10 }}>
        In diesem Run wurde der LLM-Pattern-Hint als bindend verwendet —
        Engine-Gates wurden zu reiner Telemetrie.  Eine hohe Gate-Violation-
        Rate bedeutet: der LLM-Klassifikator wählt Patterns, die das
        Regelsystem nie zugelassen hätte. Das ist nicht automatisch falsch,
        muss aber pro Cluster geprüft werden.
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 12 }}>
        <Stat label="LLM-only Turns" value={String(turns)} />
        <Stat
          label="Gate-Verletzungen"
          value={`${violations} (${(violationRate * 100).toFixed(0)}%)`}
          color={violationColor}
        />
        <Stat
          label="Saubere Wahl"
          value={`${turns - violations} (${((1 - violationRate) * 100).toFixed(0)}%)`}
          color="#10B981"
        />
      </div>
      ...
      {kinds.length > 0 && (
        <div style={{ marginBottom: 8 }}>
          <div style={{ fontSize: 11, color: '#6B7280', marginBottom: 4 }}>
            Verletzte Gate-Arten
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {kinds.map(([k, n]) => (
              <span key={k} style={{
                padding: '2px 8px', fontSize: 11, fontFamily: 'monospace',
                background: '#fff', border: '1px solid #FCD34D', borderRadius: 10,
              }}>
                {k} <span style={{ color: '#6B7280' }}>×{n}</span>
              </span>
            ))}
          </div>
        </div>
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        color: color || '#111827', fontFamily: 'monospace',
      }}>
        {value}
      </div>
    </div>
