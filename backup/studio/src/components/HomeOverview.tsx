...
export default function HomeOverview({ elements, backendOnline, onNavigate, onOpenSnapshots }: Props) {
  ...
  //     Welle E (2026-05-17): 16 Patterns, 11 Intents nach dem Refactor. ──
  // Welle E Sprint 3 (2026-05-18): 15 Patterns (P3+P4 → P3 Wissens-Frage),
  // 10 Intents (INT-PLATTFORM + INT-KONZEPT → INT-WISSEN).
  const patternCount = elements?.patterns?.length ?? 15;
  const personaCount = elements?.personas?.length ?? 8;
  const intentCount = elements?.intents?.length ?? 10;
  ...
  const layers: LayerCard[] = [
    ...
    },
    {
      ...
      icon: '\u{1F9E9}',
      label: 'Patterns',
      headline: 'LLM-Hint wählt Pattern. Engine modelliert Output (Welle E).',
      primaryCount: `${patternCount} Patterns`,
      tags: ['Plattform-Info', 'Material-Suche', 'Lernpfad', 'Canvas', 'Safety'],
      color: '#7C3AED',
    },
    {
      num: 4, id: 'dimensions',
    ...
    },
    {
      ...
      primaryCount: 'RAG-Whitelist + MCP-Tools',
      tags: ['Pattern-spezifisches RAG', 'On-Demand-only', 'MCP-Server', 'Themenseiten-Resolver'],
      color: '#D97706',
    },
  ];
      ...
      {backendOnline && (
        <div className="home-status-strip">
          ...
          >
            ...
            <div className="home-status-meta">
              {engineStats
                ? `${engineStats.rule_count} Regeln · ${engineStats.shadow_count} shadow${
                    engineStats.live_count === 0 ? ' (Welle E)' : ''
                  }`
                : 'lädt…'}
            </div>
          </button>
